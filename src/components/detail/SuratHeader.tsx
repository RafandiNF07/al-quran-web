import { QORI_LIST } from '../../constants/qori';
...
type SuratHeaderProps = {
  ...
  selectedQori: QoriId;
  onSelectQori: (qoriId: QoriId) => void;
    ...
    <header className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-sm md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <button
            onClick={() => onToggleAudio(detailSurat.audioFull[selectedQori], 0)}
          ...
          <div className="mt-4">
            <label className="mb-1 block text-xs font-medium text-gray-500">Pilih Qori:</label>
            <select
              value={selectedQori}
              onChange={(event) => onSelectQori(event.target.value as QoriId)}
              className="rounded-lg border border-emerald-100 bg-white p-2 text-sm text-emerald-900 outline-none focus:ring-2 focus:ring-emerald-500"
            >
              {QORI_LIST.map((qori) => (
